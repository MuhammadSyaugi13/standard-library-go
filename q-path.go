package main

import (
	"fmt"
	"path"
	"path/filepath"
)

func main() {
	fmt.Println(path.Dir("hello/world.go"))
	fmt.Println(path.Base("hello/world.go"))
	fmt.Println(path.Ext("hello/world.go"))
	fmt.Println(path.Join("hello", "world", "main.go"))

	/*
	* filepath dapat mengetahui perbedaan penulisan path pada windows atau os lainnya
	* contohnya pada linux menulis dengan /, sedangkan pada windos dengan \
	 */

	fmt.Println("\n==============================================\n")

	fmt.Println(filepath.Dir("hello/world.go"))
	fmt.Println(filepath.Base("hello/world.go"))
	fmt.Println(filepath.Ext("hello/world.go"))
	fmt.Println(filepath.IsAbs("D:\\Coding\\golang\\standard-libarary-go"))
	fmt.Println(filepath.IsLocal("hello/world.go"))
	fmt.Println(filepath.Join("hello", "world", "main.go")) //perbedanannya disini, mengikuti format os
}
